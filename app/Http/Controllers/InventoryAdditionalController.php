...
class InventoryAdditionalController extends Controller
{
    public function searchItems(Request $request)
    {
        ...
        if ($share_id === null) {
            $inventories = Inventory::where('user_id', $user_id)->where('name', 'like', "%$key%")->paginate(10);
            return view('inventory.index', compact('inventories'));
        }

        $inventories = Inventory::where('share_id', $share_id)->where('name', 'like', "%$key%")->paginate(10);
        return view('inventory.index', compact('inventories'));
    }
}
